...
# Alma API methods
	def self.get(uri, format=:json)
		response =
		 RestClient.get "https://api-na.hosted.exlibrisgroup.com/almaws/v1" + uri,
		 	accept: format
		return format==:json ?
	...
	def self.put(uri, data)
		response =
		 RestClient.put "https://api-na.hosted.exlibrisgroup.com/almaws/v1" + uri,
		 	data.to_json,
			accept: :json,
			authorization: "apikey " + "#{Rails.configuration.alma[:apikey]}",
			content_type: :json
	...
	def self.post(uri, data)
		response =
		 RestClient.post "https://api-na.hosted.exlibrisgroup.com/almaws/v1" + uri,
		 	data.to_json,
			accept: :json,
			authorization: "apikey " + "#{Rails.configuration.alma[:apikey]}",
			content_type: :json
	...
	def self.delete(uri)
		RestClient.delete "https://api-na.hosted.exlibrisgroup.com/almaws/v1" + uri,
			authorization: "apikey " + "#{Rails.configuration.alma[:apikey]}"
	end
	...
	def self.get_bibs_availability(mms_id, formats=["p","d","e"])
		xml = get("/bibs?apikey=#{Rails.configuration.alma[:apikey]}&mms_id=#{mms_id}&expand=" +
	...
	def self.get_items(params)
		url = "/bibs/#{params[:mms_id]}/holdings/#{params[:holding_id]}/items?apikey=#{Rails.configuration.alma[:apikey]}&expand=due_date_policy&order_by=description&direction=desc&user_id=#{params[:user_id]}&limit=50"
	...
	def self.get_request_options(params)
		opts = get "/bibs/#{params[:mms_id]}/request-options?apikey=#{Rails.configuration.alma[:apikey]}&user_id=#{params[:user_id]}"
